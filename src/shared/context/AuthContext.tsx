...
interface AuthContextValue {
  ...
  signIn: (email: string, password: string) => Promise<AcaoResultado>;
  signUp: (dados: SignUpDados) => Promise<AcaoResultado & { precisaConfirmarEmail?: boolean }>;
  signOut: () => Promise<{ error: Error | null }>;
}

const AuthContext = createContext<AuthContextValue | undefined>(undefined);

async function buscarProfile(user: User): Promise<Profile> {
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signIn = async (email: string, password: string): Promise<AcaoResultado> => {
    const { error } = await supabase.auth.signInWithPassword({ email, password });
    return error ? { ok: false, mensagem: traduzirErroAuth(error) } : { ok: true };
  };

  const signUp = async ({ email, password, nome, empresa, cargo, papel }: SignUpDados) => {
