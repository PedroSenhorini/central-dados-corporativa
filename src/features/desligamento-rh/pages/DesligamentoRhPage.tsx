...
import DesligamentoSummary, { type DesligamentoSummaryProps } from '../components/DesligamentoSummary.js';
...
import type { Papel } from '../../../shared/types/database.js';
...
export default function DesligamentoRH() {
  ...
  const [perfis, setPerfis] = useState<ColaboradorResumo[]>([]);
  ...
  const [resumo, setResumo] = useState<DesligamentoSummaryProps | null>(null);
  ...
  const toggleLicense = (id: string) => setLicenses((prev) => ({ ...prev, [id]: !prev[id] }));
  const selectedLicenseLabels = LICENCAS_M365.filter((l) => licenses[l.id]).map((l) => l.label);

  const canSubmit = Boolean(colaboradorId && dataDesligamento && status !== 'running');
