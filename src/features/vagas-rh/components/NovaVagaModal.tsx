...
import type { Papel, PrioridadeVaga } from '../../../shared/types/database.js';
...
interface NovaVagaModalProps {
  perfis: PerfilBasico[];
  responsaveisRh: PerfilBasico[];
  perfilAtualId?: string;
  onFechar: () => void;
  onCriar: (novaVaga: NovaVagaInput) => Promise<void>;
}
...
export default function NovaVagaModal({ perfis, responsaveisRh, perfilAtualId, onFechar, onCriar }: NovaVagaModalProps) {
  const [titulo, setTitulo] = useState('');
  const [setorArea, setSetorArea] = useState('');
  const [gestorId, setGestorId] = useState('');
  const [responsavelId, setResponsavelId] = useState(perfilAtualId ?? '');
  const [prioridade, setPrioridade] = useState<PrioridadeVaga>('media');
  const [prazoSlaDias, setPrazoSlaDias] = useState(30);
  const [salvando, setSalvando] = useState(false);

  const podeSalvar = Boolean(titulo.trim().length > 1 && setorArea && gestorId && prazoSlaDias > 0 && !salvando);
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select
                id="prioridade"
                value={prioridade}
                onChange={(e) => setPrioridade(e.target.value as PrioridadeVaga)}
                className={`${inputClass} bg-white`}
              >
                {PRIORIDADES.map((p) => (
