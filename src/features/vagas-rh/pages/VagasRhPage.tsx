...
import VagaCard from '../components/VagaCard.js';
import NovaVagaModal, { type NovaVagaInput, type PerfilBasico } from '../components/NovaVagaModal.js';
import { COLUNAS_VAGAS, ETAPAS_FINAIS, calcularDiasEmAberto } from '../data/constants.js';
import type { StatusVaga, VagaComRelacoes } from '../../../shared/types/database.js';
...
export default function VagasRhPage() {
  ...
  const [vagas, setVagas] = useState<VagaComRelacoes[]>([]);
  const [perfis, setPerfis] = useState<PerfilBasico[]>([]);
  ...
  const carregarVagas = async () => {
    ...
    if (error) {
      ...
      setVagas((data ?? []) as unknown as VagaComRelacoes[]);
    }
    setCarregando(false);
  };
  ...
  const podeArrastar = (vaga: VagaComRelacoes) => podeGerenciar || vaga.gestor_solicitante_id === user?.id;

  const moverVaga = async (id: string, novoStatus: StatusVaga) => {
  ...
  const criarVaga = async (novaVaga: NovaVagaInput) => {
  ...
  const handleDragStart = (e: DragEvent<HTMLDivElement>, vagaId: string) => {
    e.dataTransfer.setData('text/plain', vagaId);
  };

  const handleDrop = (e: DragEvent<HTMLDivElement>, colunaId: StatusVaga) => {
    e.preventDefault();
