...
import { Sparkles, X, TrendingUp, TrendingDown, Minus, Send, type LucideIcon } from 'lucide-react';
...
import type { Insight, TipoInsight } from '../engine/insightsEngine.js';

const ESTILO_TIPO: Record<TipoInsight, { icon: LucideIcon; badge: string }> = {
  positivo: {
    icon: TrendingUp,
...
function CartaoInsight({ titulo, mensagem, tipo }: Pick<Insight, 'titulo' | 'mensagem' | 'tipo'>) {
...
export default function AssistantWidget() {
  ...
  const area = AREAS_DASHBOARD.find((a) => a.id === areaId) ?? AREAS_DASHBOARD[0];
  ...
  const handlePerguntar = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
